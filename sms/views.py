...
# from sms.helpers import captcha_image_url
from sms.models import Captcha
...
def captcha_image(request, key, scale=1):
    try:
        store = Captcha.objects.get(hashkey=key)
...
def captcha_refresh(request, phone_num=None):
    """  Return json with new captcha for ajax refresh request """
    if not request.is_ajax() or not phone_num:
        raise Http404

    l_key = Captcha.generate_key()
    new_key = l_key[0]
    challenge = l_key[1]
    ...
    # req.sms_param = json.dumps({'code': challenge, 'product': '锐翌医学'})
    # req.rec_num = phone_num
    # req.sms_template_code = "SMS_4615060"
    # try:
