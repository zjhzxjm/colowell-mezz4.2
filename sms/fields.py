...
from sms.models import Captcha, get_safe_now
...
class SmsBaseCaptchaTextInput(MultiWidget):
    """
    ...
    def __init__(self, attrs=None):
        ...
        super(SmsBaseCaptchaTextInput, self).__init__(widgets, attrs)
...
class SmsCaptchaTextInput(SmsBaseCaptchaTextInput):
    def __init__(self, attrs=None, **kwargs):
        ...
        super(SmsCaptchaTextInput, self).__init__(attrs)

    def build_attrs(self, extra_attrs=None, **kwargs):
        ret = super(SmsCaptchaTextInput, self).build_attrs(extra_attrs, **kwargs)
        if self._args.get('id_prefix') and 'id' in ret:
            ret['id'] = '%s_%s' % (self._args.get('id_prefix'), ret['id'])
        return ret

    def id_for_label(self, id_):
        ret = super(SmsCaptchaTextInput, self).id_for_label(id_)
        if self._args.get('id_prefix') and 'id' in ret:
    ...
    def render(self, name, value, attrs=None):
        ...
        return super(SmsCaptchaTextInput, self).render(name, self._value, attrs=attrs)


class SmsCaptchaField(MultiValueField):
    def __init__(self, *args, **kwargs):
        ...
        kwargs['widget'] = kwargs.pop('widget', SmsCaptchaTextInput(
            # output_format=kwargs.pop('output_format', None),
            id_prefix=kwargs.pop('id_prefix', None)
        ))

        super(SmsCaptchaField, self).__init__(fields, *args, **kwargs)
    ...
    def clean(self, value):
        super(SmsCaptchaField, self).clean(value)
        ...
        if settings.CAPTCHA_TEST_MODE and response.lower() == 'passed':
            ...
            try:
                ...
                Captcha.objects.get(hashkey=value[0]).delete()
            ...
            try:
                Captcha.objects.get(response=response, hashkey=value[0], expiration__gt=get_safe_now()).delete()
            except Captcha.DoesNotExist:
